...
2. 克隆仓库后执行 =npm install && npm run build= 以编译扩展.
3. 运行方式(二选一):  
   - 调试: 在仓库根目录按 =F5= 启动 Extension Development Host。  
   - 已打包安装: 运行 =vsce package= 生成 VSIX,在 VS Code 里选择 "Install from VSIX..." 安装。
4. 在 Host/安装好的 VS Code 窗口设置里搜索 =DeepSeek C# Assistant=,配置 =apiUrl= 与 =apiKey= 等必需项(也可直接在 =.vscode/settings.json= 中填写示例配置,并在此处启用 =autoRunTests= 等可选开关).
5. 打开任意 .cs 文件,在需要生成代码的位置放置光标,通过命令面板执行 "DeepSeek: C# Assist" 或在右键菜单中选择同名命令.
6. 在弹出的预览中审阅生成的代码,点击 Insert 写入文件;如果在设置中已启用 =autoRunTests=,扩展会自动运行 dotnet test 并在输出与 Webview 中展示结果。
