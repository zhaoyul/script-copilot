...
2. 克隆仓库并执行 =npm install && npm run build=; 然后按 =F5= 启动 Extension Development Host(或运行 =vsce package= 后在 VS Code 里 =Install from VSIX...= 安装打包的扩展).
3. 在 Host 窗口的设置里搜索 =DeepSeek C# Assistant=,配置 =apiUrl= 与 =apiKey= 等必需项(也可直接在 =.vscode/settings.json= 中填写示例配置).
4. 打开任意 .cs 文件,在需要生成代码的位置放置光标,通过命令面板执行 "DeepSeek: C# Assist" 或在右键菜单中选择同名命令.
5. 在弹出的预览中审阅生成的代码,点击 Insert 写入文件;若启用 autoRunTests,扩展会自动运行 dotnet test 并在输出与 Webview 中展示结果。
